...
export const Question = (props) => {
  ...
  if (question) {
            ...
            <label htmlFor="optionOneRadio">
              ...
              {answer && (
                <span className="statistic">
                  {`${Math.round(
                    (opt1Votes / totalVotes) * 100
                  )}% (${opt1Votes} vote${opt1Votes !== 1 ? "s" : ""})`}
                </span>
              )}
            </label>
          </div>
          <div className="form-group">
            ...
            <label htmlFor="optionTwoRadio">
              ...
              {answer && (
                <span className="statistic">
                  {`${Math.round(
                    (opt2Votes / totalVotes) * 100
                  )}% (${opt2Votes} vote${opt2Votes !== 1 ? "s" : ""})`}
                </span>
              )}
            </label>
          </div>
          <button type="submit" value="Submit" disabled={answer ? true : false}>
            Submit
          </button>
        </form>
      </div>
    ...
    return <h1>Sorry, we could not find a matching question</h1>;
  }
};
